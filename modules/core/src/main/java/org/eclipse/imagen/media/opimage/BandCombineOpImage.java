 ...
 *
 * <p>
 * This <code>OpImage</code> performs the arbitrary interband
...
final class BandCombineOpImage extends PointOpImage {
  ...
  protected void computeRect(
    ...
    RasterAccessor d = new RasterAccessor(
        dest,
        destRect,
        formatTags[1],
        getColorModel());
    ...
    for (int h = 0; h < dheight; h++) {
      ...
      for (int w = 0; w < dwidth; w++) {
        for (int b = 0; b < dbands; b++) {
          ...
          float[] color = new float[sbands];
          for (int k = 0; k < sbands; k++) {
            color[k] += (float) (sData[k][spo + sBandOffsets[k]] & 0xFF);
          }
          for (int k = 0; k < sbands; k++) {
            sum += (float) mat[k] * color[k];
          }

          byte value = ImageUtil.clampRoundByte(sum + (float) mat[sbands]);
          dData[b][dpo + dBandOffsets[b]] = value;
        }
    ...
    for (int h = 0; h < dheight; h++) {
      ...
      for (int w = 0; w < dwidth; w++) {
        for (int b = 0; b < dbands; b++) {
          ...
          dData[b][dpo + dBandOffsets[b]] = ImageUtil
              .clampRoundUShort(sum + (float) this.matrix[b][sbands]);
        }
    ...
    for (int h = 0; h < dheight; h++) {
      ...
      for (int w = 0; w < dwidth; w++) {
        for (int b = 0; b < dbands; b++) {
          ...
          dData[b][dpo + dBandOffsets[b]] = ImageUtil
              .clampRoundInt(sum + (float) this.matrix[b][sbands]);
        }
